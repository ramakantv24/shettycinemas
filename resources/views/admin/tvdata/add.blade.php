          ...
          <div class="card">
            <div class="card-body">
              <h4 class="card-title">Tv Data <span class="card-description">
				...
				@endif
				<form role="form" action="{{ asset('admin/add_tvdata_action') }}" method="POST" enctype='multipart/form-data'>
				@CSRF
                    ...
                    <select class="js-example-basic-single w-100  @error('tv_type_id') is-invalid @enderror" name="tv_type_id">
						@if(isset($Tvtype))							
							@foreach($Tvtype as $list)
							  <option value="{{ $list->id }}">{{ $list->tv_name }}</option>							  
							@endforeach
						@endif
                    </select>					
					@error('tv_type_id')
                ...
                <div class="form-group col-6" id="videoLink" style="display:none;">                    
                    <div class="row" style="margin-top: 28px;">
                        ...
                        <div class="form-group col-6">
                            <input type="text" class="form-control @error('video_link') is-invalid @enderror" name="video_link" id="video_link" value="{{ old('video_link') }}" placeholder="Enter the video id only">
                        </div>
                    </div>                    
                </div>                
